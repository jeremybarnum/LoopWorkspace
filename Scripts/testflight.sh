#!/bin/zsh
#
# testflight.sh — headless archive -> upload for the Sport Mode on next-dev build.
#
...
# ONE SIMULATOR PAIR PER WORKING TREE. Two trees on one simulator fail each other's runs. A tree
# names its own pair in an uncommitted `.sims.env` at its root (SIM_ID=…, WATCH_SIM_ID=…);
# without one, the defaults below are the port tree's pair.
[[ -f "$ROOT/.sims.env" ]] && source "$ROOT/.sims.env"
SIM_ID="${SIM_ID:-70631DFE-9079-4EE5-B541-14CB75F280C5}"   # "Port-NextDev" — THIS line's own sim. BE1EB8F5 belongs to
...
WATCH_SIM_ID="${WATCH_SIM_ID:-6F1A0ECD-08FD-49F0-91D9-5E8C3E21FE6A}"   # "Port-NextDev Watch", watchOS 26.5 (matches the
...
# half of this port that has never run, so a green ship with no watch coverage would be the most
# misleading outcome available. Set ALLOW_NO_WATCH_GATE=1 to ship without it, knowingly.
run_watch_gate() {
  : > "$WATCH_RUNLOG"
  if ! xcodebuild -workspace "$ROOT/LoopWorkspace.xcworkspace" -list 2>/dev/null | grep -q "WatchAppTests"; then
    if [[ "${ALLOW_NO_WATCH_GATE:-0}" == "1" ]]; then
      note "WATCH GATE SKIPPED — no WatchAppTests target on this branch (ALLOW_NO_WATCH_GATE=1)"
      return 0
    fi
    note "WATCH GATE UNAVAILABLE — WatchAppTests has not been ported to this branch."
    note "  The watch half of this port has never been executed. Port the target, or"
    note "  re-run with ALLOW_NO_WATCH_GATE=1 to ship without watch coverage."
    return 68
  fi
  local rc=0
  ready_sims
